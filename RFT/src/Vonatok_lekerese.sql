...
WHERE DATE (START + INTERVAL DISTANCE_BEFORE_START / SPEED * 60.0 MINUTE) = DATE ('2016-12-10')
	AND START + INTERVAL DISTANCE_BEFORE_START / SPEED * 60.0 MINUTE > NOW()
	...
	FROM (
		SELECT *
			...
			IF (
				...
				FROM WAGONS
				GROUP BY WAGONS.TRAINS_ID
				) TMP14 ON TMP10.SECOND_TRAIN_ID = TMP14.TMP14_TRAINS_ID WHERE DATE (FIRST_TRAIN_START + INTERVAL DISTANCE_BEFORE_START1 / FIRST_TRAIN_SPEED * 60.0 MINUTE) = DATE ('2016-12-10')
				AND FIRST_TRAIN_START + INTERVAL DISTANCE_BEFORE_START1 / FIRST_TRAIN_SPEED * 60.0 MINUTE > NOW()
