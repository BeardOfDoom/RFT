SELECT *, START + INTERVAL DISTANCE_BEFORE_START/SPEED*60.0 MINUTE AS 'START_TIME', START + INTERVAL (DISTANCE_BEFORE_START+DISTANCE_BETWEEN_START_AND_STOP)/SPEED*60.0 MINUTE AS 'ARRIVE_TIME'
FROM WAGONS
INNER JOIN 
	...
	FROM
		...
		INNER JOIN 
				...
				(SELECT ROUTE_STATIONS_CONNECTION.ROUTES_ID , SUM(ROUTE_STATIONS_CONNECTION.EXPLETIVE_TICKET) AS 'TMP3_EXPLETIVE_TICKET', ROUTE_STATIONS_CONNECTION.TYPE_OF_TRAINS_STOPS AS 'FROM_TYPE_OF_TRAINS_STOPS', TMP.TO_TYPE_OF_TRAINS_STOPS, TMP.STATIONS_ID, TMP.TMP2_STATIONS_ID, TMP.STATION_INDEX_IN_ROUTE, TMP.TMP2_STATION_INDEX_IN_ROUTE
					...
					INNER JOIN
						...
						INNER JOIN 
							...
							FROM ROUTE_STATIONS_CONNECTION
							INNER JOIN STATIONS ON ROUTE_STATIONS_CONNECTION.STATIONS_ID = STATIONS.ID
							WHERE STATIONS.NAME = 'Kisvárda') TMP2
							ON ROUTE_STATIONS_CONNECTION.ROUTES_ID = TMP2.TMP2_ROUTES_ID
						WHERE STATIONS.NAME = 'Ajak' AND TMP2.TMP2_STATION_INDEX_IN_ROUTE > ROUTE_STATIONS_CONNECTION.STATION_INDEX_IN_ROUTE) TMP 
			...
			WHERE DATE(TRAIN_ROUTE_CONNECTION.START) = DATE('2016-12-02') AND TRAIN_ROUTE_CONNECTION.START > NOW() AND TMP3.TMP3_EXPLETIVE_TICKET = 0 AND TMP3.FROM_TYPE_OF_TRAINS_STOPS LIKE CONCAT('%',TRAINS.TYPE,'%') AND TMP3.TO_TYPE_OF_TRAINS_STOPS LIKE CONCAT('%',TRAINS.TYPE,'%')) TMP4
