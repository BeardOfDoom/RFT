...
WHERE DATE (START + INTERVAL DISTANCE_BEFORE_START / SPEED * 60.0 MINUTE) = DATE ('2016-12-14')
	...
	FROM (
		SELECT *
			...
			IF (
				...
				FROM WAGONS
				GROUP BY WAGONS.TRAINS_ID
				) TMP14 ON TMP10.SECOND_TRAIN_ID = TMP14.TMP14_TRAINS_ID WHERE DATE (FIRST_TRAIN_START + INTERVAL DISTANCE_BEFORE_START1 / FIRST_TRAIN_SPEED * 60.0 MINUTE) = DATE ('2016-12-14')
				...
				-- EXAMPLE WHERE ((CLASS1_SERVICES1 LIKE '%1%' OR CLASS2_SERVICES1 LIKE '%1%') AND (CLASS1_SERVICES2  LIKE '%1%' OR CLASS2_SERVICES2  LIKE '%1%'))
				...
				AND TMP16.STATION_INDEX_IN_ROUTE BETWEEN TMP_ATSZALLAS.MIDDLE_STATION_INDEX_IN_ROUTE2
					...
					GROUP BY ID2,ID3) TMP_ALL
WHERE FIRST_TRAIN_EXPLETIVE_TICKET = 0 AND SECOND_TRAIN_EXPLETIVE_TICKET = 0
ORDER BY START_TIME1